...
**Requirements:** JDK 21. Jackson is the only runtime dependency.
...
Add the jar to your project, or depend on the built artifact from your local `~/.m2` repository after `mvn install`.
...
Adding a new search mode = new sub-package implementing `TrieRecallStrategy` + one `SearchMode` enum value + one line in `SearchStrategies`. Full package map and algorithm detail: [HLD.md](https://github.com/vatsal259/docube-db/blob/main/HLD.md) in the repo.
...
- **No infrastructure tax** - plain files, one jar, JDK 21. Drop a folder on disk and go.
...
| **README** (API reference, quick start) | [github.com/vatsal259/docube-db#readme](https://github.com/vatsal259/docube-db#readme) |
| **High-Level Design** (algorithms, on-disk format, complexity) | [github.com/vatsal259/docube-db/blob/main/HLD.md](https://github.com/vatsal259/docube-db/blob/main/HLD.md) |
...
Questions, bug reports, or ideas for a new search mode? [Open an issue](https://github.com/vatsal259/docube-db/issues) on GitHub.
